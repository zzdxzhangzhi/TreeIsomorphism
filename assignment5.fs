...
let Layout (t: RootedTree<int>) = 
    let rec preorderGRD l (d: int) (t: RootedTree<int>) =        
        //seq {  
        let currentL = Seq.append l (Seq.singleton d)
        match t with
        | Leaf _ -> currentL
        | RootedTree (_, children) ->
            loop currentL (d + 1) children 
             //for child in children do
             //    preorderGRD currentL (d + 1) child
        | None -> seq []
        //}

    and loop l (d: int) (s: seq<RootedTree<int>>) =
        if Seq.isEmpty s then l
        else
            let hd1, tl1 = Seq.head s, Seq.tail s
            let currentL = preorderGRD l d hd1
            loop currentL d tl1            
...
//            //    else false
//            let rec compareTreeSeqs tSeq1 tSeq2 =
//                if Seq.isEmpty tSeq1 then true
//                else 
//                    let hd1, tl1 = Seq.head tSeq1, Seq.tail tSeq1
//                    if not (compareWithSeq tSeq2 hd1) then false
//                    else compareTreeSeqs tl1 tSeq2
            
//            compareTreeSeqs children1 children2
//            //let seqResult = Seq.map (compareWithSeq children2) children1 
//            //if Seq.exists (fun x -> x = false) seqResult then false
//            //else true

//and compareWithSeq (tSeq: seq<RootedTree<int>>) t = 
//    if Seq.isEmpty tSeq then false
//    else
//        let hd, tl = Seq.head tSeq, Seq.tail tSeq
//        if (IsLayoutEqual hd t) then true    
//        else compareWithSeq tl t

let rec IsIsomorphism (t1: RootedTree<int>) (t2: RootedTree<int>) =  
    match (t1, t2) with 
    | (None, None) | (Leaf _, Leaf _) -> true
    | (None, _) | (_, None) | (Leaf _, _) | (_, Leaf _) -> false
    | (RootedTree _, RootedTree _) ->        
        let children1 = getChildren t1
        let children2 = getChildren t2
        let cN1 = Seq.length children1
        let cN2 = Seq.length children2
...
and compareWithSeq (tSeq: seq<RootedTree<int>>) t = 
    if Seq.isEmpty tSeq then false
        ...
        let hd, tl = Seq.head tSeq, Seq.tail tSeq
        if (IsIsomorphism hd t) then true    
        else compareWithSeq tl t
    ...
    let rec readInputData treesNum scenarioCount =
        ...
        if treesNum = 0 then ()
            ...
            for j in 1 .. treesNum - 1 do
                let index = Array.sub trees 0 j |> Array.tryFindIndex (IsIsomorphism trees.[j])
